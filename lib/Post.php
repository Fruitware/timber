...
class Post extends Core implements CoreInterface {
	...
	/**
	 * Get the CSS classes for a post, but with caching css post classes. For usage you should use `{{post.class}}` instead of `{{post.post_class}}`
	 * @internal
	 * @param string $class additional classes you want to add
	 * @see \Timber\Post::$_post_class
	 * @example
	 * ```twig
	...
	public function get_class( $class = '' ) {
		if (!$this->_post_class) {
			$this->_post_class = $this->post_class();
		}

		return sprintf('%s %s', $this->_post_class, $class);
	}
